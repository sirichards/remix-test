 ...
 * @package Dewynters Headless 
 */
function dewynters_admin_bar_menu($admin_bar) {
...
add_action('admin_bar_menu', 'dewynters_admin_bar_menu', 100);
 ...
 * @package Dewynters Headless 
 */
function dewynters_load_custom_wp_admin($hook_suffix) {

    wp_register_script( 'dewynters_admin_scripts', get_stylesheet_directory_uri() . '/assets/js/admin.js', array('jquery'), '1.0.1', false );
    wp_register_style( 'dewynters_admin_css', get_template_directory_uri() . '/assets/css/admin.css', false, '1.0.1' );
    wp_enqueue_style( 'dewynters_admin_css' );

    wp_enqueue_script( 'dewynters_admin_scripts' );

    wp_localize_script( 'dewynters_admin_scripts', 'dewynters_vars', array(
            'nonce'    => wp_create_nonce( 'dewynters_publish_changes' ),
            'ajax_url' => admin_url( 'admin-ajax.php' ),
        )
    );
}
add_action( 'admin_enqueue_scripts', 'dewynters_load_custom_wp_admin' );
 ...
 * @package Dewynters Headless 
 */
function dewynters_build() {
    check_ajax_referer( 'dewynters_publish_changes', 'security' );
...
add_action('wp_ajax_dewynters_build', 'dewynters_build');
 ...
 * 
 */
// function dewynters_build_on_save($post_id) {
...
//     }
// }
// add_action('save_post', 'dewynters_build_on_save');