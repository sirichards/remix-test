...
if ( $user->user_login!='admin' && $user->user_login!='s.richards' && $user->user_login!='d.simon' && $user->user_login!='user' ) {
	add_filter('acf/settings/show_admin', '__return_false');
}


if (!function_exists('dewynters_nullify_empty')) {
    /**
     ...
     * @return mixed
     */
    function dewynters_nullify_empty($value, $post_id, $field) {
        if (empty($value)) {
            return null;
        }
        return $value;
    ...
    add_filter('acf/format_value', 'dewynters_nullify_empty', 100, 3);
}
 ...
 */
function dewynters_hide_dummy_ticket( $args, $field, $post_id ) {
    $args['post__not_in'] = array( 506 );//'Dummy ACF content'    
    return $args;
}
add_filter( 'acf/fields/relationship/query/key=field_5e307cb2d50d4', 'dewynters_hide_dummy_ticket', 10, 3 );

/**
 * hide the dummy acf ticket form the tickets relationship field in blocks
 */
function dewynters_hide_dummy_sidebar( $args, $field, $post_id ) {
    $args['post__not_in'] = array( 510 );//'Dummy ACF content'    
    return $args;
}
add_filter( 'acf/fields/relationship/query/key=field_5d5a8e825e89d', 'dewynters_hide_dummy_sidebar', 10, 3 );
...
function my_mce4_options($init) {
    ...
    $custom_colours =  '"000089", "HPWW Navy",
                        "8ADAF1", "HPWW Cyan",
                        "EBCA44", "HPWW Yellow"';
